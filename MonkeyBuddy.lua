...
function MonkeyBuddy_OnEvent(event)
	if event == "VARIABLES_LOADED" then
		if myAddOnsList then myAddOnsList[MONKEYBUDDY_TITLE] = {name = MONKEYBUDDY_TITLE, description = MONKEYBUDDY_DESCRIPTION, version = MONKEYBUDDY_VERSION, frame = "MonkeyBuddyIconButton", optionsframe = "MonkeyBuddyFrame"} end
		if not MonkeyBuddyConfig then MonkeyBuddyConfig = {} MonkeyBuddyConfig.m_bDismissed = false end
...
function MonkeyBuddyQuestTab_OnClick()
	if MonkeyQuest and MonkeyQuest.m_bLoaded then
...
function MonkeyBuddySpeedTab_OnClick()
	if MonkeySpeed and MonkeySpeed.m_bLoaded then
...
function MonkeyBuddyClockTab_OnClick()
	if MonkeyClock and MonkeyClock.m_bLoaded then
		MonkeyBuddyQuestFrame:Hide()
		MonkeyBuddySpeedFrame:Hide()
...
function MonkeyBuddyCheckButton_OnClick()
	if this:GetChecked() then
		this.pSlashCommand(true)
	else
		this.pSlashCommand(false)
	end
end
